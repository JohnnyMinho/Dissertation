#include "LocationPingWarnerD2D.h"
...
Define_Module(LocationPingWarnerD2D)

static const simsignal_t storyboardSignal = cComponent::registerSignal("StoryboardSignal");

LocationPingWarnerD2D::~LocationPingWarnerD2D()
{
    cancelAndDelete(removeSpeedReduction);
}

int LocationPingWarnerD2D::numInitStages() const
{
    return inet::NUM_INIT_STAGES;
}

void LocationPingWarnerD2D::initialize(int stage)
{
...
void LocationPingWarnerD2D::finish()
{
    socket.close();
...
void LocationPingWarnerD2D::receiveSignal(cComponent*, simsignal_t sig, cObject* obj, cObject*)
{
...
void LocationPingWarnerD2D::handleMessage(cMessage* msg)
{
...
void LocationPingWarnerD2D::sendReport()
{
    Enter_Method_Silent();
...
void LocationPingWarnerD2D::processReport(BlackIceReport& report)
{
    using boost::units::si::meter;
